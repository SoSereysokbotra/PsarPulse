...
export default function AdminSupportPage() {
  ...
  const selectUser = (user: any) => {
    ...
    fetch(`/api/admin/users/${user.id}/notes`).then(r => r.json()).then(d => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className={`text-2xl font-bold tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}>Support Notes</h1>
        <p className={`text-sm mt-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>Add internal notes to user accounts for support purposes.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[calc(100vh-220px)]">
        {/* User list */}
        <div className={`rounded-xl border flex flex-col overflow-hidden ${isDark ? "bg-[#161b22] border-white/10" : "bg-white border-slate-200"}`}>
          <div className={`p-3 border-b ${isDark ? "border-white/5" : "border-slate-100"}`}>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
              <input type="text" placeholder="Find user..." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && fetchUsers()}
          ...
          <div className="flex-1 overflow-y-auto">
            {users.map(user => (
              <button key={user.id} onClick={() => selectUser(user)}
                ...
                <p className={`text-sm font-medium ${isDark ? "text-slate-200" : "text-slate-800"}`}>{user.fullName}</p>
          ...
          {!selectedUser ? (
            <div className="flex-1 flex items-center justify-center p-8 text-center">
              <div>
                ...
                <p className="text-sm text-slate-400">Select a user to view and add support notes</p>
              </div>
            </div>
          ) : (
            <>
              <div className={`px-4 py-3 border-b ${isDark ? "border-white/5" : "border-slate-100"}`}>
                <p className={`font-semibold text-sm ${isDark ? "text-white" : "text-slate-800"}`}>{selectedUser.fullName}</p>
                <p className="text-xs text-slate-500">{selectedUser.email}</p>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {loadingNotes ? (
                  <div className="flex justify-center pt-8"><Loader2 className="animate-spin text-slate-400 w-5 h-5" /></div>
                ) : notes.length === 0 ? (
                  <p className="text-sm text-slate-400 text-center pt-8">No notes yet</p>
                ) : notes.map(note => (
                  <div key={note.id} className={`rounded-lg p-3 ${isDark ? "bg-white/5" : "bg-slate-50"}`}>
                    <p className={`text-sm ${isDark ? "text-slate-200" : "text-slate-700"}`}>{note.note}</p>
              ...
              <div className={`p-3 border-t ${isDark ? "border-white/5" : "border-slate-100"}`}>
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={addNote} disabled={postingNote || !newNote.trim()}
                    ...
                    {postingNote ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />}
                  </button>
                </div>
              </div>
            </>
