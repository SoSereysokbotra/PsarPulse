...
import {
  ...
  PanelLeftClose, PanelLeftOpen, Info, BarChart3
} from "lucide-react";
...
interface SummaryCardProps { title: string; khmerTitle: string; value: string; icon: React.ElementType; trend?: string; isPositive?: boolean; subtext?: string; highlight?: boolean; tooltipText?: string; }
...
export default function DemoDashboard() {
  ...
  // ─── DEMO STATIC DATA ───
  const demoSummary  = { sales: "$124.50", expenses: "$45.00", profit: "$79.50", customers: "42", avgCustomer: "$2.96" };
  ...
  const [timeData, setTimeData] = useState({ greeting: "Welcome", greetingKh: "សួស្តី", dateStr: "Loading..." });
  ...
  useEffect(() => {
    ...
    setTimeData({
      ...
      greetingKh: hour < 12 ? "អរុណសួស្តី" : hour < 17 ? "ទិវាសួស្តី" : "សាយ័ណ្ហសួស្តី",
      dateStr: now.toLocaleDateString("en-KH", { weekday: "long", month: "long", day: "numeric" })
    });
  }, []);
  ...
  return (
    <TooltipContext.Provider value={setTooltip}>
      ...
      <div className="flex h-screen w-full overflow-hidden bg-[#f0f2f5] text-[#111827] pt-[36px]" style={{ fontFamily: "inherit" }}>
        ...
        {/* ── Custom Floating Tooltip (FIXED POSITIONING) ── */}
        <div 
        ...
        >
          ...
          <p className="font-medium text-white mb-1.5 flex items-center gap-1.5"><Info size={12}/> Demo Insight</p>
          <p>{tooltip.content}</p>
        </div>
        ...
        <aside
          className={`
            fixed lg:static inset-y-0 left-0 z-50 pt-[36px] lg:pt-0
        ...
        >
          ...
          <nav className={`flex-1 pt-3 overflow-y-auto ${isSidebarCollapsed ? "px-2" : "px-3"}`}>
            <NavItem icon={LayoutDashboard}  title="Dashboard" khmerTitle="ផ្ទាំងគ្រប់គ្រង" href="/guest"         active collapsed={isSidebarCollapsed} />
            ...
            <NavItem icon={Users}            title="Customers" khmerTitle="អតិថិជន"          href="/guest/customers"    collapsed={isSidebarCollapsed} />
          </nav>
            ...
            {!isSidebarCollapsed && (
              <>
                ...
                <div className="relative mt-3 pt-3 border-t border-white/[0.07]">
                  <button
                    onClick={() => window.location.href = "/auth"}
                    className="w-full flex items-center gap-3 px-3 pb-1 bg-transparent border-0 text-left cursor-pointer hover:opacity-80 transition-opacity"
                  >
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#d1d5db] to-[#9ca3af] flex items-center justify-center text-[13px] font-bold text-[#0d1117] shrink-0">
                      GU
                    </div>
        ...
        <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
          ...
          <header className="bg-white border-b border-[#e8eaed] px-5 lg:px-7 h-[70px] flex items-center justify-between shrink-0 relative z-30">
            <div className="flex items-center gap-3">
              ...
              <h1 className="font-bold text-[20px] text-[#111827] hidden sm:block">Overview</h1>
            </div>
            ...
            <div className="flex items-center gap-[10px] sm:gap-[14px]">
                ...
                {quickSaleOpen && (
                  ...
                  >
                    ...
                    <div className="p-[14px_18px]">
                      ...
                      {/* Complete sale — Converted to PLG hook */}
                      <button
                        onMouseDown={e => { 
                          e.preventDefault(); 
                          if (cart.length) {
                            showToast("Sale recorded! Create an account to save permanently.");
                            closeQuickSale();
                          }
                        }}
              ...
              <div className="w-[34px] h-[34px] rounded-full bg-[rgba(156,163,175,0.12)] border-[1.5px] border-[#9ca3af] flex items-center justify-center text-[11px] font-bold text-[#9ca3af] cursor-pointer hover:bg-[rgba(156,163,175,0.2)] transition-colors"
                   onClick={() => window.location.href = "/auth"}>
                GU
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 lg:px-9 py-[26px]">
            <div className="max-w-[1400px] mx-auto flex flex-col gap-5">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-baseline gap-2">
                      <span className="text-[18px] font-extrabold text-[#e6edf3]">{timeData.greeting}, Guest User!</span>
                    </div>
              ...
              <div 
                ...
                onMouseEnter={(e) => setTooltip({ content: "Simplified usage tracker. Real accounts get unlimited daily logs.", x: e.clientX, y: e.clientY })}
                onMouseLeave={() => setTooltip({ content: null, x: 0, y: 0 })}
              >
                <div className="flex items-center justify-between mb-[10px]">
                  <div>
                    <div className="text-[13.5px] font-semibold text-[#111827]">Demo Sales Logs</div>
                    <div className="text-[11px] text-[#6b7280] mt-px">គំរូនៃកំណត់ត្រាលក់ប្រចាំខែ</div>
                  </div>
                  <span className="text-sm font-bold text-[#111827]">
                    {demoUsage.used} <span className="text-[#6b7280] font-normal">/ {demoUsage.limit}</span>
                  </span>
                </div>
                <div className="w-full h-1.5 bg-[#f0f2f5] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${demoUsagePct}%`, background: demoUsagePct > 80 ? "#f59e0b" : "#3ecf8e" }}
                  />
                </div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <SummaryCard title="Total Sales" khmerTitle="ការលក់សរុប" value={demoSummary.sales} icon={CircleDollarSign} trend="+12%" isPositive 
                  tooltipText="Track real-time income from processed sales orders. Unlock this tracking with an account." />
                <SummaryCard title="Total Expenses" khmerTitle="ចំណាយសរុប" value={demoSummary.expenses} icon={Receipt} trend="-5%" isPositive 
                  tooltipText="Track all operational costs automatically. Create an account to start logging." />
                <SummaryCard title="Net Profit" khmerTitle="ប្រាក់ចំណេញ" value={demoSummary.profit} icon={TrendingUp} trend="+18%" isPositive highlight 
                  tooltipText="Net Profit auto-calculated. Unlock accurate historical profit trends by signing up." />
                <SummaryCard title="Customers" khmerTitle="អតិថិជនសរុប" value={demoSummary.customers} icon={Users} subtext={`Avg ${demoSummary.avgCustomer}`}
                  tooltipText="Customer transaction volume. Build customer loyalty programs when you upgrade." />
              </div>
                  ...
                  <div className="h-[180px] flex items-end gap-[7px]">
                    {monthlyData.map((v, i) => (
                      <div key={i} className="flex-1 flex flex-col items-center gap-[5px]">
                        ...
                        <span className="text-[10px] text-[#6b7280]">Week {i + 1}</span>
                      </div>
                    ))}
                  </div>
                </ChartCard>
              ...
              <div className="bg-white border border-[#e8eaed] rounded-[14px] px-[26px] py-[22px]">
                ...
                <div className="flex flex-col gap-[16px] mb-[24px]">
                  {EXP_BREAKDOWN.map(item => (
                    <div key={item.key} className="flex items-center gap-4 cursor-help"
                         onMouseEnter={(e) => setTooltip({ content: `Unlock dynamic expense categorization for ${item.labelEn} with an account.`, x: e.clientX, y: e.clientY })}
                         onMouseLeave={() => setTooltip({ content: null, x: 0, y: 0 })}>
                ...
                <div className="pt-[18px] border-t border-[#f0f2f5]">
                  ...
                  <div className="flex flex-wrap gap-3 items-end">
                    ...
                    />
                    <button 
                      onClick={() => showToast("Expense logged! Create an account to permanently save to your records.")}
                ...
                <div className="px-[26px] py-[22px]">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 mb-[18px]">
                    <div className="px-5 py-[18px] bg-white/[0.05] rounded-xl text-center border border-white/[0.07]">
                      ...
                      <div className="font-bold text-[26px] text-[#e6edf3]">{demoSummary.sales}</div>
                    </div>
                    <div className="px-5 py-[18px] bg-[rgba(239,68,68,0.08)] rounded-xl text-center border border-[rgba(239,68,68,0.15)]">
                      ...
                      <div className="font-bold text-[26px] text-[#ef4444]">{demoSummary.expenses}</div>
                    </div>
                    <div className="px-5 py-[18px] bg-[rgba(62,207,142,0.10)] rounded-xl text-center border border-[rgba(62,207,142,0.20)]">
                      ...
                      <div className="font-bold text-[26px] text-[#3ecf8e]">{demoSummary.profit}</div>
                    </div>
                  </div>

                  <div className="text-[12.5px] text-[#7d8590] mb-[18px]">
                    Auto-calculated (Demo data): {demoSummary.sales} − {demoSummary.expenses} ={" "}
                    <strong className="text-[#e6edf3]">{demoSummary.profit}</strong>
                  </div>

                  <button
                    onClick={() => showToast("Day confirmed! Sign up to generate your final accounting report.")}
                  ...
                  >
                    ...
                    Confirm & Lock Day
                    <span className="text-[11px] opacity-75 ml-1">បញ្ជាក់ និងចាក់សោ</span>
                  </button>
                </div>
              </div>
...
function SummaryCard({ title, khmerTitle, value, icon: Icon, trend, isPositive, subtext, highlight = false, tooltipText }: SummaryCardProps) {
  const setTooltip = useContext(TooltipContext);

  return (
    <div 
      className={`px-[26px] py-6 rounded-[14px] shadow-[0_1px_4px_rgba(0,0,0,0.06)] border cursor-help transition-transform hover:-translate-y-0.5 ${
        highlight ? "bg-[#3ecf8e] border-[#3ecf8e]" : "bg-white border-[#e8eaed]"
      }`}
      onMouseEnter={(e: MouseEvent<HTMLDivElement>) => {
        if (tooltipText) setTooltip({ content: tooltipText, x: e.clientX, y: e.clientY });
      }}
      onMouseLeave={() => setTooltip({ content: null, x: 0, y: 0 })}
    >
      <div className="flex justify-between items-start mb-5">
        <div>
          <div className={`text-[10.5px] font-bold uppercase tracking-[0.07em] ${highlight ? "text-[#0d1117]" : "text-[#6b7280]"}`}>
            {title}
          </div>
          <div className={`text-[10px] mt-0.5 ${highlight ? "text-[#0d1117]/70" : "text-[#9ca3af]"}`}>
            {khmerTitle}
          </div>
        </div>
        <div className={`w-[34px] h-[34px] rounded-[10px] flex items-center justify-center border ${
          ...
          <Icon size={15} className={highlight ? "text-[#0d1117]" : "text-[#3ecf8e]"} />
        </div>
      </div>
      <div className="flex items-end justify-between">
        <span className={`font-bold text-[30px] leading-none ${highlight ? "text-[#0d1117]" : "text-[#111827]"}`}>
          {value}
        </span>
        {trend && (
          <span className={`text-[12.5px] font-bold mb-0.5 ${
            highlight ? "text-[#0d1117]" : isPositive ? "text-[#3ecf8e]" : "text-[#ef4444]"
          }`}>
            {trend}
          </span>
        )}
        {subtext && (
          <span className={`text-[12.5px] mb-0.5 ${highlight ? "text-[#0d1117]/75" : "text-[#6b7280]"}`}>
            {subtext}
          </span>
        )}
      </div>
...
function ChartCard({ title, khmer, children, tooltipText }: { title: string; khmer: string; children: React.ReactNode, tooltipText?: string }) {
  const setTooltip = useContext(TooltipContext);
