...
const navItems = [
  ...
  { href: "/superadmin/invitations", label: "Invitations", icon: MailOpen },
  {
...
export default function SuperadminLayout({
  ...
  return (
    ...
    >
      {/* Sidebar */}
      <aside
        className={`w-64 min-h-screen shrink-0 border-r flex flex-col ${isDark ? "bg-[#161b22] border-white/10" : "bg-white border-slate-200"}`}
      >
        {/* Logo */}
        <div
          className={`p-5 border-b ${isDark ? "border-white/10" : "border-slate-100"}`}
        >
        ...
        <nav className="flex-1 p-3 space-y-0.5">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`w-4 h-4 shrink-0 ${isActive ? "text-white" : "text-slate-400 group-hover:text-current"}`}
                />
        ...
        {/* Footer */}
        <div
          className={`p-4 border-t ${isDark ? "border-white/10" : "border-slate-100"}`}
        >
          <Link
            href="/admin"
            className={`flex items-center gap-2 text-xs font-medium ${isDark ? "text-slate-500 hover:text-slate-300" : "text-slate-400 hover:text-slate-600"} transition-colors`}
          >
