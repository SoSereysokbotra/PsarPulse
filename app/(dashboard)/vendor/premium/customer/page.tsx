      ...
      <div className="flex-1 overflow-y-auto px-6 md:px-8 py-6 space-y-6 transition-colors">
        <div className="pt-1 pb-1">
          ...
          <p className="text-[14px] text-[#6b7280] dark:text-[#7d8590] mt-1">
            {isKhmer ? "តាមដានចរាចរណ៍ និងគ្រប់គ្រងភាពស្មោះត្រង់" : "Track traffic and manage loyalty"} ·{" "}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          <VendorSummaryCard 
            ...
            subtext={isKhmer ? `${summaryData.todayLogs} កំណត់ត្រា` : `${summaryData.todayLogs} log entries`} 
          />
          <VendorSummaryCard 
            ...
            <div className="bg-white dark:bg-[#0d1117] border border-[#e8eaed] dark:border-white/10 rounded-2xl p-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6 transition-colors">
              <div>
                ...
                <p className="text-[12px] text-slate-500 dark:text-[#7d8590] mt-1">
                  {isKhmer ? "កត់ត្រាចំនួនអតិថិជនដែលមកភ្លាមៗ" : "Immediately record incoming groups"}
                </p>
              </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-white/5 text-[11px] uppercase font-bold text-slate-500 dark:text-[#7d8590] tracking-wider transition-colors">
                      <th className="px-6 py-4">{isKhmer ? "ម៉ោងបញ្ជូល" : "Time Entry"}</th>
                      <th className="px-6 py-4">{isKhmer ? "ចំនួនអតិថិជន" : "Group Size"}</th>
                      <th className="px-6 py-4">{isKhmer ? "ស្ថានភាពចរាចរណ៍" : "Traffic Status"}</th>
                      <th className="px-6 py-4 text-right">{isKhmer ? "សកម្មភាព" : "Action"}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-white/5 transition-colors">
                    {loading ? (
                       <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-400">{isKhmer ? "កំពុងទាញយក..." : "Loading traffic logs..."}</td></tr>
                    ) : filteredLogs.length === 0 ? (
                       <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-400">{isKhmer ? "មិនឃើញមានកំណត់ត្រាទេ។" : "No logs found."}</td></tr>
                    ) : (
                      filteredLogs.map((log) => (
                        <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-6 py-4 font-bold text-[15px] dark:text-white">+{log.count} {isKhmer ? "នាក់" : "Persons"}</td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex px-3 py-1 rounded-full text-[11px] font-bold border ${log.count >= 10 ? "bg-orange-50 border-orange-100 text-orange-600" : "bg-emerald-50 border-emerald-100 text-emerald-600"}`}>
                              {log.count >= 10 ? (isKhmer ? "មមាញឹកខ្លាំង" : "Peak Traffic") : (isKhmer ? "ធម្មតា" : "Regular")}
                            </span>
                          </td>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-4 bg-white dark:bg-[#0d1117] border border-[#e8eaed] dark:border-white/10 rounded-2xl shadow-sm p-6 space-y-5 h-fit transition-colors">
              <div>
                ...
                <p className="text-[12px] text-slate-500 dark:text-[#7d8590] mt-1">
                  {isKhmer ? "រក្សាទុកព័ត៌មានសម្រាប់ភាពស្មោះត្រង់ និងការបញ្ចុះតម្លៃ" : "Save details for loyalty & discounts"}
                </p>
              </div>
              <form onSubmit={handleCRMSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[12px] font-bold text-slate-700 dark:text-[#9aa4b2] ml-1">{isKhmer ? "ឈ្មោះពេញ" : "Full Name"}</label>
                  <input required placeholder={isKhmer ? "ឧ. ចាន់ ណារ៉េត" : "e.g. John Doe"} value={crmName} onChange={e => setCrmName(e.target.value)} className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 outline-none focus:border-psar-primary transition-colors text-sm dark:text-white" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[12px] font-bold text-slate-700 dark:text-[#9aa4b2] ml-1">{isKhmer ? "លេខទូរស័ព្ទ" : "Phone Number"}</label>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-white/5 text-[11px] uppercase font-bold text-slate-500 dark:text-[#7d8590] transition-colors">
                      <th className="px-6 py-4">{isKhmer ? "ព័ត៌មានអតិថិជន" : "Customer Details"}</th>
                      <th className="px-6 py-4">{isKhmer ? "ភាពស្មោះត្រង់" : "Loyalty Status"}</th>
                      <th className="px-6 py-4">{isKhmer ? "កំណើនថ្មីៗ" : "Recent Growth"}</th>
                      <th className="px-6 py-4 text-right">{isKhmer ? "សកម្មភាព" : "Actions"}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-white/5 transition-colors">
                    {loading ? (
                       <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-400">{isKhmer ? "កំពុងទាញយក..." : "Loading profiles..."}</td></tr>
                    ) : filteredCRM.length === 0 ? (
                      <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-400">{isKhmer ? "មិនទាន់មានអតិថិជនចុះឈ្មោះនៅឡើយទេ។" : "No customers registered yet."}</td></tr>
                    ) : (
                        ...
                        <tr key={customer.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                          ...
                          <td className="px-6 py-4">
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-600 text-[11px] font-extrabold shadow-sm">
                              <Heart className="w-3 h-3" /> {isKhmer ? "អតិថិជនប្រចាំ" : "Regular"}
                            </span>
                          </td>
                          <td className="px-6 py-4">
        ...
        {activeTab === "analysis" && (
          <CustomerAnalytics logHistory={logHistory} crmDatabase={crmDatabase} todaySales={todaySales} />
        )}
      </div>
...
function CustomerAnalytics({ logHistory, crmDatabase, todaySales }: { logHistory: any[]; crmDatabase: any[]; todaySales: number }) {
  const { language } = useLanguage();
  const isKhmer = language === "km";
  ...
  const retentionRate = crmDatabase.length > 0
    ? Math.min(100, Math.round((logHistory.length / (crmDatabase.length * 3)) * 100))
    : 0;

  const totalVisitors = logHistory.reduce((s, l) => s + (l.count || 0), 0);
  const avgPerVisit = totalVisitors > 0 ? (todaySales / Math.max(1, logHistory.filter(l => new Date(l.createdAt).toDateString() === new Date().toDateString()).reduce((s, l) => s + (l.count || 0), 0))).toFixed(2) : "0.00";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-[#0d1117] border border-[#e8eaed] dark:border-white/10 rounded-2xl p-5 text-center transition-colors">
          <div className="text-[28px] font-black text-[#111827] dark:text-white">{totalVisitors}</div>
        ...
        <div className="bg-white dark:bg-[#0d1117] border border-[#e8eaed] dark:border-white/10 rounded-2xl p-5 text-center transition-colors">
          <div className="text-[28px] font-black text-[#8b5cf6]">{crmDatabase.length}</div>
          <div className="text-[11px] font-bold text-[#6b7280] dark:text-[#7d8590] uppercase tracking-wider mt-1">{isKhmer ? "ព័ត៌មានដែលបានចុះឈ្មោះ" : "Registered Profiles"}</div>
        </div>
        <div className="bg-white dark:bg-[#0d1117] border border-[#e8eaed] dark:border-white/10 rounded-2xl p-5 text-center transition-colors">
          <div className="text-[28px] font-black text-[#f59e0b]">${avgPerVisit}</div>
          <div className="text-[11px] font-bold text-[#6b7280] dark:text-[#7d8590] uppercase tracking-wider mt-1">{isKhmer ? "មធ្យមការចំណាយថ្ងៃនេះ" : "Avg. Spend Today"}</div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-[#0d1117] border border-[#e8eaed] dark:border-white/10 rounded-2xl p-6 transition-colors">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="font-bold text-[16px] text-[#111827] dark:text-white flex items-center">
              {isKhmer ? "ផែនទីកំដៅម៉ោងមមាញឹកដែលបានព្យាករណ៍" : "Predicted Peak Hour Heatmap"} 
              <span className="text-[10px] text-[#8b5cf6] leading-none font-bold tracking-wider border border-[#8b5cf6]/30 bg-[#8b5cf6]/10 px-2 py-1 rounded-full ml-2">MODEL</span>
            </h3>
          </div>
            ...
            {hourlyData.slice(6, 22).map((count, i) => {
              ...
              return (
                <div key={hour} className="flex flex-col items-center gap-1">
                  ...
                  />
                  <span className="text-[8px] text-[#9ca3af] dark:text-[#7d8590]">{hour % 12 || 12}{hour >= 12 ? "p" : "a"}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex items-end gap-2 h-32">
            {dailyVisits.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                ...
                <span className="text-[10px] text-[#9ca3af] dark:text-[#7d8590]">{d.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[#0d1117] rounded-2xl border border-white/10 p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="font-bold text-[16px] text-white">{isKhmer ? "ការវិភាគអតិថិជន AI" : "AI Customer Insights"}</h3>
            <p className="text-[11px] text-[#7d8590]">Powered by Gemini · {isKhmer ? "ការវិភាគអតិថិជន AI" : "AI Customer Insights"}</p>
          </div>
        </div>
        {insightsLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {aiInsights.map((insight, idx) => (
              <div key={idx} className="p-4 bg-white/[0.04] border border-white/[0.07] rounded-xl hover:bg-white/[0.07] transition-colors">
                <span
                ...
                <h4 className="font-bold text-[14px] text-white mb-1">{insight.title}</h4>
                <p className="text-[12px] text-[#7d8590] leading-relaxed">{insight.detail}</p>
              </div>
            ))}
          </div>
        ) : (
          <div className="text-center py-8">
            <p className="text-[#7d8590] text-[13px]">
              {isKhmer ? "កត់ត្រាទិន្នន័យចរាចរណ៍បន្ថែមទៀត ដើម្បីទទួលបានការវិភាគអតិថិជនដោយ AI។" : "Log more traffic data to unlock AI-powered customer insights."}
            </p>
          </div>
        )}
      </div>
