      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-7 transition-colors">
        <div className="pt-1 pb-2">
          <h2 className="text-[32px] font-extrabold text-[#111827] dark:text-white leading-tight">
            {isKhmer ? "ស្តុករបស់ខ្ញុំ" : "My Inventory"}
          </h2>
          <p className="text-[14px] text-[#6b7280] dark:text-[#7d8590] mt-1">
            {isKhmer ? "តាមដាន និងគ្រប់គ្រងស្តុកទំនិញរបស់អ្នក" : "Track and manage your product inventory"} ·{" "}
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
          <div onClick={() => setFilterStatus("all")} className="cursor-pointer">
            <VendorSummaryCard title={isKhmer ? "ផលិតផលសរុប" : "Total Products"} khmerTitle="ផលិតផលសរុប" value={summaryData.totalItems} icon={Package} isPositive={true} highlight={filterStatus === "all"} />
          </div>
          <div onClick={() => setFilterStatus("low")} className="cursor-pointer">
            <VendorSummaryCard title={isKhmer ? "ស្តុកជិតអស់" : "Low Stock"} khmerTitle="ស្តុកជិតអស់" value={summaryData.lowStock} icon={AlertTriangle} isPositive={false} highlight={filterStatus === "low"} />
          </div>
          <VendorSummaryCard title={isKhmer ? "តម្លៃស្តុក" : "Inventory Value"} khmerTitle="តម្លៃស្តុក" value={summaryData.totalValue} icon={CircleDollarSign} />
          <VendorSummaryCard title={isKhmer ? "លក់ដាច់បំផុត" : "Top Seller"} khmerTitle="លក់ដាច់បំផុត" value={summaryData.mostSold} icon={TrendingUp} />
          <VendorSummaryCard title={isKhmer ? "អត្រាលក់ចេញ" : "Turnover Rate"} khmerTitle="អត្រាលក់ចេញ" value={summaryData.stockTurnover} icon={ArrowUpRight} isPositive={true} />
        </div>
          ...
          <div className="p-5 md:p-6 border-b border-[#f0f2f5] dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors">
            <div>
              <h3 className="font-bold text-[17px] text-slate-900 dark:text-white">
                {isKhmer ? "បញ្ជីផលិតផល" : "Product List"}
              </h3>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              <button onClick={() => { setEditingItem(null); setFormData({name:"", khmerName:"", price:"", stock:0, threshold:10}); setIsModalOpen(true); }} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-psar-primary text-white font-bold px-4 py-2.5 rounded-xl hover:opacity-90 transition-opacity min-h-[44px] border-0 cursor-pointer">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-white/5 border-b border-[#f0f2f5] dark:border-white/5 text-[11px] text-slate-500 dark:text-[#7d8590] uppercase tracking-wider font-semibold transition-colors">
                  <th className="px-6 py-4">{isKhmer ? "ព័ត៌មានផលិតផល" : "Product Details"}</th>
                  <th className="px-6 py-4">{isKhmer ? "តម្លៃ / ថ្លៃដើម" : "Price / Cost"}</th>
                  <th className="px-6 py-4">{isKhmer ? "កម្រិតស្តុក" : "Stock Level"}</th>
                  <th className="px-6 py-4">{isKhmer ? "ការបញ្ចូលចុងក្រោយ" : "Threshold"}</th>
                ...
                {filteredItems.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[11px] text-slate-400 mt-0.5">{isKhmer ? "ថ្លៃដើម" : "Cost"}: $1.00</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold ${item.status === 'out' ? 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400' : item.status === 'low' ? 'bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400' : 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400'}`}>
                        {item.status === 'out' ? (isKhmer ? "អស់ពីស្តុក" : "Out of Stock") : item.status === 'low' ? (isKhmer ? "ស្តុកជិតអស់" : "Low Stock") : (isKhmer ? "មានក្នុងស្តុក" : "In Stock")}
                      </span>
                    </td>
          ...
          {(filterStatus !== "all" || searchTerm !== "") && (
            <div className="p-4 border-t border-[#f0f2f5] dark:border-white/5 bg-slate-50 dark:bg-[#161B22] text-center">
              <button onClick={() => { setSearchTerm(""); setFilterStatus("all"); }} className="inline-flex items-center gap-2 px-6 py-2 bg-[#8b5cf6]/10 hover:bg-[#8b5cf6]/20 text-[#8b5cf6] font-bold rounded-full transition-all border-0 cursor-pointer text-[13px]">
                <X className="w-4 h-4" /> {isKhmer ? "សម្អាត" : "Clear Filters"}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          ...
          <div className="bg-white dark:bg-[#0d1117] p-6 md:p-8 rounded-[24px] shadow-2xl w-full max-w-md relative z-10 border border-[#e8eaed] dark:border-white/10 transition-colors">
            ...
            <h3 className="text-[22px] font-bold mb-6 text-[#111827] dark:text-white">
              {editingItem ? (isKhmer ? "កែសម្រួលទំនិញ" : "Edit Inventory Item") : (isKhmer ? "បន្ថែមទំនិញថ្មី" : "Add New Item")}
            </h3>
            {error && <div className="mb-4 p-3 bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 text-xs font-bold rounded-lg">{isKhmer ? "មិនអាចរក្សាទុកបានទេ" : error}</div>}
