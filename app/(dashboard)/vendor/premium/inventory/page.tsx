...
export default function InventoryPage() {
  ...
  const lowStockCount = inventoryItems.filter(i => i.status !== "good").length;
  const lowStockNames = inventoryItems
    .filter(i => i.status !== "good")
    .map(i => language === 'km' && i.khmerName ? i.khmerName : i.name);
  ...
  return (
    <VendorDashboardLayout
      ...
      rightActions={
        <>
          <button className="hidden sm:flex items-center gap-2 bg-psar-dark hover:opacity-90 text-white dark:bg-white dark:text-psar-dark font-medium px-4 py-2 rounded-xl transition-colors text-[13px] min-h-[40px] cursor-pointer border-0">
            <Download className="w-4 h-4" /> {t("dashboard.actions.exportStock")}
          </button>
        </>
      }
    >
      <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-7 transition-colors">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1.5">
                  <h3 className="font-bold text-[16px] text-orange-900 dark:text-orange-400">
                    {t("inventory.alerts.lowStockTitle")} ({lowStockCount})
                  </h3>
                </div>
                <p className="text-orange-800 dark:text-orange-300/80 text-[14px] leading-relaxed max-w-2xl">
                  <strong>{lowStockNames.join(", ")}</strong> {t("inventory.alerts.lowStockDesc")}
                </p>
              </div>
            </div>
            <button className="w-full md:w-auto px-5 py-2.5 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-xl transition-colors text-sm shadow-sm min-h-[44px] z-10 flex items-center justify-center gap-2 border-0 cursor-pointer">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <VendorSummaryCard
            title={t("dashboard.metrics.totalProducts")}
            khmerTitle="ផលិតផលសរុប"
            value={summaryData.totalItems}
            icon={Package}
            isPositive={true}
            trend="Premium Unlimited"
          />
          <VendorSummaryCard
          ...
          />
          <VendorSummaryCard
            title={t("dashboard.metrics.topSeller")}
        ...
        <div className="bg-white dark:bg-[#0d1117] border border-slate-200 dark:border-white/5 rounded-2xl shadow-sm overflow-hidden flex flex-col transition-colors">
          <div className="p-5 md:p-6 border-b border-slate-100 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3">
              <div className="relative w-full sm:w-auto group">
                <Search className="w-4 h-4 text-slate-400 dark:text-[#7d8590] absolute left-3 top-1/2 -translate-y-1/2 group-focus-within:text-psar-primary transition-colors" />
                <input
                  type="text"
                  placeholder={t("inventory.searchPlaceholder")}
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-[#161B22] border border-slate-200 dark:border-white/5 rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:border-psar-primary focus:ring-1 focus:ring-psar-primary min-h-[44px] transition-colors placeholder:text-slate-400 dark:placeholder:text-[#7d8590] outline-none"
                />
              </div>
              <button className="w-full sm:w-auto flex items-center justify-center gap-2 bg-psar-primary text-white font-bold px-4 py-2.5 rounded-xl hover:bg-psar-primary/90 transition-all min-h-[44px] shadow-sm border-0 cursor-pointer">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-[#161B22] border-b border-slate-100 dark:border-white/5 text-[11px] text-slate-500 dark:text-[#7d8590] uppercase tracking-wider font-bold transition-colors">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5 transition-colors">
                {inventoryItems.map((item) => (
                  <tr
                    key={item.id}
                    className="hover:bg-psar-primary/5 dark:hover:bg-white/5 transition-colors group"
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          className="p-2 text-slate-400 dark:text-[#7d8590] hover:text-psar-primary dark:hover:text-psar-primary rounded-lg hover:bg-psar-primary/10 transition-colors border-0 bg-transparent cursor-pointer"
                          title="Quick Restock"
                        >
                          <PlusCircle className="w-4 h-4 mx-auto" />
                        </button>
                        <button
                          className="p-2 text-slate-400 dark:text-[#7d8590] hover:text-slate-900 dark:hover:text-white rounded-lg hover:bg-slate-100 dark:hover:bg-white/10 transition-colors border-0 bg-transparent cursor-pointer"
                          title="Edit Product"
                        >
          ...
          <div className="p-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-[#161B22] text-center transition-colors">
            <button className="text-[13px] font-semibold text-psar-primary hover:underline border-0 bg-transparent cursor-pointer">
              {t("dashboard.actions.viewAll")} {summaryData.totalItems} {t("dashboard.metrics.categories")}
            </button>
          </div>
        </div>
      </div>
