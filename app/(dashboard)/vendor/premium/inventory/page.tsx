      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-7 transition-colors">
            ...
            <div className="flex items-start gap-4 z-10">
              ...
              <div>
                ...
                <p className="text-orange-800 dark:text-orange-300/80 text-[14px] leading-relaxed max-w-2xl">
                  <strong>{lowStockItems.map((i) => i.name).slice(0, 3).join(", ")}{lowStockItems.length > 3 ? "..." : ""}</strong> {lowStockItems.length === 1 ? "is" : "are"} running low. Consider restocking soon.
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
          <div onClick={() => setFilterStatus("all")} className="cursor-pointer">
            <VendorSummaryCard title="Total Products" khmerTitle="ផលិតផលសរុប" value={summaryData.totalItems} icon={Package} isPositive={true} trend={filterStatus === "all" ? "Selected" : "View all"} highlight={filterStatus === "all"} />
          </div>
          <div onClick={() => setFilterStatus("low")} className="cursor-pointer">
            <VendorSummaryCard title="Low Stock" khmerTitle="ស្តុកជិតអស់" value={summaryData.lowStock} icon={AlertTriangle} trend={filterStatus === "low" ? "Filtering..." : "Needs attention"} isPositive={false} highlight={filterStatus === "low"} />
          </div>
          <VendorSummaryCard title="Inventory Value" khmerTitle="តម្លៃស្តុក" value={summaryData.totalValue} icon={CircleDollarSign} trend="Capital locked" />
          <VendorSummaryCard title="Top Seller" khmerTitle="លក់ដាច់បំផុត" value={summaryData.mostSold} icon={TrendingUp} subtext="Highest margin" />
          <VendorSummaryCard title="Turnover Rate" khmerTitle="អត្រាលក់ចេញ" value={summaryData.stockTurnover} icon={ArrowUpRight} trend="Fast moving" isPositive={true} />
        </div>
        ...
        <div className="bg-white dark:bg-[#0d1117] border border-slate-200 dark:border-white/5 rounded-2xl shadow-sm overflow-hidden flex flex-col transition-colors">
          <div className="p-5 md:p-6 border-b border-slate-100 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors">
            <div>
              ...
              <p className="text-[13px] font-khmer text-slate-500 dark:text-[#7d8590] mt-0.5">បញ្ជីផលិតផល</p>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              <button onClick={openAdd} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-[#8b5cf6] to-[#3ecf8e] text-white font-medium px-4 py-2.5 rounded-xl hover:opacity-90 transition-colors cursor-pointer min-h-[44px] border-0 shadow-sm">
                <Plus className="w-4 h-4" /> <span className="text-sm">Add Item</span>
              </button>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/5 transition-colors">
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-400 dark:text-[#7d8590]">No items found. Add some products!</td></tr>
                ) : (
                  filteredItems.map((item) => (
                    <tr key={item.id} className="hover:bg-psar-primary/10 dark:hover:bg-white/5 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex flex-col max-w-[200px] sm:max-w-[300px]">
                          <span className="text-[14px] font-bold text-slate-900 dark:text-white break-words">{item.name}</span>
                          <span className="text-[12px] font-khmer text-slate-500 dark:text-[#7d8590] break-words">{item.khmerName || "-"}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap"><span className="text-[15px] font-bold text-slate-700 dark:text-[#e6edf3]">${parseFloat(item.price).toFixed(2)}</span></td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="w-24 h-1.5 bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden transition-colors">
                            <div className={`h-full rounded-full ${item.status === "out" || item.stock === 0 ? "bg-red-500" : item.status === "low" || item.stock <= item.threshold ? "bg-orange-500" : "bg-gradient-to-r from-[#8b5cf6] to-[#3ecf8e]"}`} style={{ width: `${Math.min((item.stock / (item.threshold * 2)) * 100, 100)}%` }} />
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-[13px] font-medium text-slate-600 dark:text-[#e6edf3]">{item.threshold}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {item.status === "out" || item.stock === 0 ? (
                          <span className="inline-flex items-center px-2.5 py-1 rounded-md bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400 text-[11px] font-bold transition-colors">Out of Stock</span>
                        ) : item.status === "low" || item.stock <= item.threshold ? (
                          <span className="inline-flex items-center px-2.5 py-1 rounded-md bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400 text-[11px] font-bold transition-colors">Low Stock</span>
                        ) : (
                          <span className="inline-flex items-center px-2.5 py-1 rounded-md bg-emerald-100 dark:bg-[rgba(62,207,142,0.15)] text-emerald-700 dark:text-[#3ecf8e] text-[11px] font-bold transition-colors">In Stock</span>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openEdit(item)} className="p-2 text-slate-400 dark:text-[#7d8590] hover:text-slate-900 dark:hover:text-white rounded-lg hover:bg-slate-100 dark:hover:bg-white/10 transition-colors border-0 cursor-pointer bg-transparent"><Edit2 className="w-4 h-4 mx-auto" /></button>
                          <button onClick={() => handleDelete(item.id)} className="p-2 text-slate-400 dark:text-[#7d8590] hover:text-red-500 dark:hover:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors border-0 cursor-pointer bg-transparent"><X className="w-4 h-4 mx-auto" /></button>
                        </div>
                      </td>
                    </tr>
                  ))
          ...
          {(filterStatus !== "all" || searchTerm !== "") && (
            <div className="p-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-[#161B22] text-center transition-all animate-in fade-in slide-in-from-bottom-2">
              <button onClick={() => { setSearchTerm(""); setFilterStatus("all"); }} className="inline-flex items-center gap-2 px-6 py-2 bg-[#8b5cf6]/10 hover:bg-[#8b5cf6]/20 text-[#8b5cf6] font-bold rounded-full transition-all border-0 cursor-pointer text-[13px]">
                <X className="w-4 h-4" /> Clear All Filters (Viewing {filteredItems.length} of {inventoryItems.length} items)
              </button>
            </div>
          )}
        </div>
          ...
          <div className="bg-white dark:bg-[#0d1117] p-6 md:p-8 rounded-[24px] shadow-2xl w-full max-w-md relative z-10 border border-[#e8eaed] dark:border-white/10 transition-colors" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-slate-100 dark:border-white/10">
                ...
                <button type="submit" disabled={isSaving} className="px-5 py-3 bg-gradient-to-r from-[#8b5cf6] to-[#3ecf8e] text-white font-bold rounded-xl shadow-md hover:opacity-90 transition-opacity border-0 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed min-w-[120px]">{isSaving ? "Saving..." : editingItem ? "Save Changes" : "Confirm Item"}</button>
              </div>
            </form>
          </div>
        </div>
