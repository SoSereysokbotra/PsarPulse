...
const PREMIUM_NAV = [
  { icon: LayoutDashboard,  title: "Dashboard",  khmerTitle: "ផ្ទាំងគ្រប់គ្រង", href: "/vendor/premium",          active: true },
...
export default function PremiumDashboard() {
  ...
  const closeQuickSale = React.useCallback(() => {
    setQuickSaleOpen(false); setCart([]); setSearchQuery("");
  }, []);
  ...
  const filteredProducts = PRODUCT_LIBRARY.filter((p) => p.name.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
          ...
          <div ref={quickSaleRef} className="relative">
              ...
              >
                ...
                {cartItems > 0 && !quickSaleOpen && (
                  <span className="bg-[#0E1319] text-[#29B28D] rounded-full w-[18px] h-[18px] text-[10px] font-extrabold flex items-center justify-center ml-0.5">{cartItems}</span>
                )}
              </button>

              {quickSaleOpen && (
                <div className="absolute top-[calc(100%+10px)] right-0 w-[330px] bg-white border border-[#e8eaed] rounded-[14px] shadow-[0_20px_56px_rgba(0,0,0,0.18)] overflow-hidden" style={{ zIndex: 9999 }}
                  ...
                  <div className="p-[14px_18px] max-h-[60vh] overflow-y-auto">
                          ...
                          {PRODUCT_LIBRARY.map((p) => {
                            ...
                            return (
                              <button key={p.id} onMouseDown={(e) => { e.preventDefault(); addToCart(p); }}
                                ...
                                {inCart && <span className="absolute top-1.5 right-2 bg-[#29B28D] text-[#0E1319] rounded-full w-[17px] h-[17px] text-[9px] font-extrabold flex items-center justify-center">{inCart.qty}</span>}
                              </button>
                            );
                          })}
                        </div>
                        ...
                        {cart.map((item) => (
                          <div key={item.product.id} className="flex items-center gap-2 py-[7px] border-b border-[#f0f2f5]">
                            ...
                            <div className="flex items-center bg-[#f7f8fa] border border-[#e8eaed] rounded-[7px] overflow-hidden shrink-0">
                              <button onMouseDown={(e) => { e.preventDefault(); changeQty(item.product.id, -1); }} className="w-6 h-6 bg-transparent border-0 cursor-pointer flex items-center justify-center text-[#6b7280] hover:bg-[#e8eaed]"><Minus className="w-3 h-3" /></button>
                              <span className="text-xs font-bold min-w-[18px] text-center">{item.qty}</span>
                              <button onMouseDown={(e) => { e.preventDefault(); changeQty(item.product.id, 1); }} className="w-6 h-6 bg-transparent border-0 cursor-pointer flex items-center justify-center text-[#6b7280] hover:bg-[#e8eaed]"><Plus className="w-3 h-3" /></button>
                            </div>
                    ...
                    <div className="flex items-center justify-between py-2 border-t border-[#f0f2f5] mb-3 mt-1">
                      ...
                      <div className="flex items-center bg-[#f7f8fa] border border-[#e8eaed] rounded-[8px] overflow-hidden">
                        <button onMouseDown={(e) => { e.preventDefault(); setCustomers((c) => Math.max(1, c - 1)); }} className="w-7 h-7 bg-transparent border-0 cursor-pointer text-[#6b7280] flex items-center justify-center hover:bg-[#e8eaed]"><Minus className="w-3.5 h-3.5" /></button>
                        <span className="text-[13px] font-bold min-w-[24px] text-center">{customers}</span>
                        <button onMouseDown={(e) => { e.preventDefault(); setCustomers((c) => c + 1); }} className="w-7 h-7 bg-transparent border-0 cursor-pointer text-[#6b7280] flex items-center justify-center hover:bg-[#e8eaed]"><Plus className="w-3.5 h-3.5" /></button>
                      </div>
                    </div>
                    ...
                    <button onMouseDown={(e) => { e.preventDefault(); if (cart.length) completeSale(); }} disabled={cart.length === 0}
                      className={`w-full py-3 font-bold text-[13.5px] border-0 rounded-[10px] flex items-center justify-center gap-[7px] transition-all ${
                        cart.length ? "bg-[#29B28D] text-[#0E1319] cursor-pointer shadow-[0_4px_14px_rgba(41,178,141,0.28)] hover:opacity-90" : "bg-[#f0f2f5] text-[#6b7280] cursor-not-allowed"
                      }`}>
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-7 max-w-7xl mx-auto w-full">
          ...
          {smartAlerts.length > 0 && (
            <div className="space-y-3">
              {smartAlerts.map((alert, i) => (
                <div key={i} className={`flex items-center gap-4 p-4 rounded-[11px] border shadow-sm transition-all hover:shadow-md ${
                  alert.type === "warning" ? "bg-orange-50 border-orange-200" : "bg-[rgba(41,178,141,0.06)] border-[rgba(41,178,141,0.2)]"
                }`}>
                  <div className={`p-2 rounded-[8px] ${alert.type === "warning" ? "bg-orange-100" : "bg-[rgba(41,178,141,0.12)]"}`}>
                    {alert.type === "warning"
                      ? <TrendingDown className="w-4 h-4 text-orange-600" />
                      : <Zap className="w-4 h-4 text-[#29B28D]" />}
                  </div>
                  <div className="flex-1">
                    <p className="text-[14px] font-bold text-[#111827]">{alert.message}</p>
                    <p className="text-[11px] text-[#6b7280] mt-0.5">{alert.time}</p>
                  </div>
                  <button className="text-[12px] font-bold text-[#6b7280] hover:text-[#111827] px-3 py-1.5 rounded-[8px] hover:bg-white transition-colors bg-transparent border-0 cursor-pointer">
                    Dismiss
                  </button>
                </div>
              ))}
          ...
          <div className="bg-[#0E1319] rounded-[14px] p-6 md:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 border border-white/[0.06] overflow-hidden relative">
            ...
            <div className="flex items-center gap-5 relative z-10">
              <div className="w-14 h-14 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-[19px] font-extrabold text-white shrink-0">SM</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <VendorSummaryCard title="Total Sales"   khmerTitle="ការលក់សរុប"   value={summaryData.sales}        icon={CircleDollarSign} trend="+24%" isPositive />
            <VendorSummaryCard title="Net Profit"    khmerTitle="ប្រាក់ចំណេញ" value={summaryData.profit}       icon={TrendingUp}       trend="+32%" isPositive highlight />
            <VendorSummaryCard title="Profit Margin" khmerTitle="អត្រាចំណេញ"  value={summaryData.profitMargin} icon={ArrowUpRight}     trend="+5.1%" isPositive />
            <VendorSummaryCard title="Customers"     khmerTitle="អតិថិជន"      value={summaryData.customers}   icon={Users}            trend="+18%" isPositive />
          </div>
                ...
                {monthlyData.map((val, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <div className="w-full bg-[rgba(41,178,141,0.12)] rounded-t-[7px] relative group" style={{ height: "100px" }}>
                      <div className="absolute bottom-0 w-full bg-[#29B28D] rounded-t-[7px] transition-all duration-500 group-hover:opacity-80" style={{ height: `${(val / 900) * 100}%` }} />
                    </div>
              ...
              <div className="space-y-2">
                {inventoryItems.filter((i) => i.status !== "good").map((item) => (
                  <div key={item.id} className="flex items-center justify-between py-2 px-3 bg-white rounded-[10px] border border-orange-100">
                    ...
                    <span className={`text-[12px] font-bold ${item.status === "out" ? "text-red-600" : "text-orange-600"}`}>{item.stock} left</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white border border-[#e8eaed] rounded-[14px] shadow-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-[#f0f2f5] flex items-center justify-between">
              <div>
                <h3 className="font-bold text-[17px] text-[#111827]">End-of-Day AI Summary</h3>
                <p className="text-[12px] text-[#6b7280] mt-0.5">សង្ខេបចុងថ្ងៃរហ័ស</p>
              </div>
              {isDayLocked && (
            ...
            <div className="p-6">
              ...
              <div className="bg-[rgba(41,178,141,0.06)] rounded-[11px] p-5 mb-6 border border-[rgba(41,178,141,0.15)]">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-bold text-[14px] text-[#111827]">Gemini Insight</h4>
                  <span className="text-[10px] font-bold text-[#29B28D] bg-[rgba(41,178,141,0.12)] px-2 py-0.5 rounded-full">PREMIUM</span>
                </div>
              ...
              {!isDayLocked ? (
                <button onClick={() => setIsDayLocked(true)}
      ...
      {showCustomCategoryModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#0E1319]/40 backdrop-blur-sm p-4">
          ...
          <div className="bg-white rounded-[14px] w-full max-w-md p-6 shadow-2xl relative z-10">
            <button onClick={() => setShowCustomCategoryModal(false)} className="absolute top-4 right-4 text-[#6b7280] hover:text-[#111827] p-1.5 rounded-[8px] hover:bg-[#f7f8fa] transition-colors bg-transparent border-0 cursor-pointer">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" placeholder="e.g., Marketing (ទីផ្សារ)" value={customCategoryName} onChange={(e) => setCustomCategoryName(e.target.value)}
              ...
              <button onClick={() => { setShowCustomCategoryModal(false); setCustomCategoryName(""); }}
      ...
      {isChatOpen && (
        <div className="fixed bottom-6 right-6 z-50 w-[380px] max-h-[520px] bg-white rounded-[14px] shadow-2xl border border-[#e8eaed] flex flex-col overflow-hidden">
          <div className="px-5 py-4 bg-[#0E1319] text-white flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-[#29B28D]">Online · Dashboard Hub</p>
              </div>
            </div>
            <button onClick={() => setIsChatOpen(false)} className="text-white/70 hover:text-white p-1 hover:bg-white/10 rounded-[8px] transition-colors bg-transparent border-0 cursor-pointer">
              <X className="w-5 h-5" />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#f7f8fa] min-h-[280px]">
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] px-4 py-3 rounded-[12px] text-[13px] leading-relaxed relative ${
                  msg.role === "user"
                    ? "bg-[#29B28D] text-[#0E1319] rounded-br-sm"
                    : "bg-white border border-[#e8eaed] text-[#111827] rounded-tl-sm shadow-sm"
                }`}>
                  {msg.role !== "user" && <Sparkles className="w-3 h-3 text-[#29B28D] absolute -top-1 -left-1" />}
                  {msg.text}
                </div>
              </div>
            ))}
          ...
          <div className="p-3 border-t border-[#e8eaed] bg-white">
            <div className="flex items-center gap-2">
              <input type="text" placeholder="Ask your assistant..." value={chatMessage} onChange={(e) => setChatMessage(e.target.value)}
                className="flex-1 px-4 py-2.5 bg-[#f7f8fa] border border-[#e8eaed] rounded-[10px] text-[13px] focus:bg-white focus:border-[#29B28D] outline-none transition-all placeholder:text-[#6b7280]"
                style={{ fontFamily: "inherit" }} />
