...
import {
  ...
  TrendingUp, TrendingDown, FileBarChart, Sparkles, Brain,
  AlertTriangle, Target, Zap, Activity, MessageSquare, Send,
  FileText, FileSpreadsheet, ArrowUpRight, ArrowDownRight,
  CloudRain, ShoppingCart, RefreshCw, Download, X,
  MousePointerClick, Flame,
} from "lucide-react";
...
const PREMIUM_NAV = [
  ...
  { icon: FileBarChart,     title: "Reports",     khmerTitle: "របាយការណ៍",        href: "/vendor/premium/reports",  active: true },
];
...
export default function PremiumReportsPage() {
  ...
  const handleSend = () => {
    ...
    setChatMsg("");
    setTimeout(() =>
      setChatMessages(p => [...p, { role: "assistant", text: "Based on your data, Saturday 12–2PM is your peak window with 35% of weekly revenue. I recommend stocking up Hot Latte ingredients tonight — 94% chance of sellout given tonight's rainy forecast." }])
    , 900);
  };
      ...
      rightActions={
        <div className="flex items-center gap-2">
          <select
            ...
            className="hidden sm:block bg-[#f7f8fa] border border-[#e8eaed] text-[13px] font-medium rounded-[10px] px-3 py-2 outline-none focus:border-[#3ecf8e] cursor-pointer"
          >
            <option>Today</option>
          ...
          <button className="hidden sm:flex items-center gap-1.5 bg-[#0d1117] text-white border-0 rounded-[10px] px-3.5 py-[9px] font-semibold text-[13px] cursor-pointer hover:bg-[#1a2332] transition-colors">
            <FileText size={14} /> PDF
          </button>
          <button className="hidden sm:flex items-center gap-1.5 bg-white border border-[#e8eaed] text-[#111827] rounded-[10px] px-3.5 py-[9px] font-semibold text-[13px] cursor-pointer hover:bg-[#f0f2f5] transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-7">
        ...
        <div className="pt-1 pb-2">
          ...
          <p className="text-[14px] text-[#6b7280] mt-1">
            ...
            <span className="text-[#9ca3af]">ការវិភាគអាជីវកម្ម AI</span>
          </p>
        </div>
        ...
        {/* ═══ 1. PROFIT & LOSS (reuses PLCard from pro/reports) ═══ */}
        {activeTab === "pl" && (
          <section>
            {/* Section header — reused from pro/reports */}
            <div className="flex items-center gap-2 mb-5">
              <div className="w-1.5 h-6 bg-[#3ecf8e] rounded-full" />
              <h2 className="font-bold text-[19px] text-[#111827]">Profit &amp; Loss</h2>
              <span className="text-[12px] text-[#9ca3af] ml-1">ចំណេញ និង ខាត</span>
            </div>

            {/* Break-even tracker — premium exclusive */}
            <div className="bg-[#0d1117] rounded-[14px] border border-white/[0.06] px-[26px] py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            {/* PLCard metric cards (reused from pro/reports) — 4 cards for premium */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
              <PLCard
                title="Total Revenue" khmer="ចំណូលសរុប"
                value="$2,450.00" trend="+22%" isPositive={true}
                icon={<TrendingUp className="w-5 h-5" />}
                accentColor="bg-[#3ecf8e]"
              />
              <PLCard
                title="Total Expenses" khmer="ចំណាយសរុប"
                value="$890.50" trend="-2%" isPositive={true}
                icon={<TrendingDown className="w-5 h-5" />}
                accentColor="bg-slate-500"
              />
              <PLCard
                title="Net Profit" khmer="ប្រាក់ចំណេញសុទ្ធ"
                value="$1,559.50" trend="+28%" isPositive={true}
                icon={<CircleDollarSign className="w-5 h-5" />}
                accentColor="bg-[#3ecf8e]"
                highlight
              />
              {/* Premium exclusive — Profit Margin */}
              <PLCard
                title="Profit Margin" khmer="អត្រាប្រាក់ចំណេញ"
                value="63.6%" trend="Excellent" isPositive={true}
        ...
        {activeTab === "sales" && (
          <section>
            <div className="flex items-center gap-2 mb-5">
              <div className="w-1.5 h-6 bg-[#3ecf8e] rounded-full" />
              <h2 className="font-bold text-[19px] text-[#111827]">Sales Report</h2>
              <span className="text-[12px] text-[#9ca3af] ml-1">របាយការណ៍ការលក់</span>
            </div>
        ...
        {/* ═══ 3. WEATHER INTEL (premium exclusive) ═══ */}
        {activeTab === "weather" && (
          <section>
            <div className="flex items-center gap-2 mb-5">
              <div className="w-1.5 h-6 bg-[#3ecf8e] rounded-full" />
              <h2 className="font-bold text-[19px] text-[#111827]">Weather Intel</h2>
              <span className="text-[12px] text-[#9ca3af] ml-1">ការព្យាករណ៍អាកាសធាតុ</span>
            </div>
            ...
            <div className="bg-[#0d1117] rounded-[14px] border border-white/[0.06] px-[26px] py-5 flex flex-col sm:flex-row sm:items-center gap-5">
              <div className="flex items-center gap-5">
                <div className="w-16 h-16 rounded-[14px] bg-[rgba(59,130,246,0.15)] border border-[rgba(59,130,246,0.2)] flex items-center justify-center shrink-0">
                  <CloudRain size={28} className="text-[#3b82f6]" />
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-[12.5px] text-[#7d8590]">🌡️ 28°C</span>
                    <span className="text-[12.5px] text-[#7d8590]">💧 82%</span>
                    <span className="text-[12.5px] text-[#7d8590]">💨 14 km/h</span>
                  </div>
                </div>
              </div>
        ...
        {/* ═══ 4. AI FORECAST (premium exclusive) ═══ */}
        {activeTab === "forecast" && (
          <section>
            <div className="flex items-center gap-2 mb-5">
              <div className="w-1.5 h-6 bg-[#3ecf8e] rounded-full" />
              <h2 className="font-bold text-[19px] text-[#111827]">AI Forecast</h2>
              <span className="text-[12px] text-[#9ca3af] ml-1">ការព្យាករណ៍ AI</span>
            </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-[#f0f2f5]">
                      {["Product", "Today", "Predicted", "Change", "Confidence"].map(h => (
                        <th key={h} className="pb-3 text-[10.5px] font-bold text-[#9ca3af] uppercase tracking-[0.07em] pr-6">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {forecastRows.map((row, i) => (
                      <tr key={i} className={i < forecastRows.length - 1 ? "border-b border-[#f0f2f5]" : ""}>
                        ...
                        <td className="py-3.5 pr-6">
                          <span className={`inline-flex items-center gap-1 text-[13px] font-bold ${row.up ? "text-[#3ecf8e]" : "text-[#ef4444]"}`}>
                            {row.up ? <ArrowUpRight size={13} /> : <ArrowDownRight size={13} />}{row.change}
                          </span>
                        </td>
                        <td className="py-3.5">
                          <div className="flex items-center gap-2">
                            <div className="w-[80px] h-1.5 bg-[#f0f2f5] rounded-full overflow-hidden">
                              <div className="h-full bg-[#3ecf8e] rounded-full" style={{ width: `${row.confidence}%` }} />
                            </div>
                            <span className="text-[12px] text-[#6b7280] font-medium">{row.confidence}%</span>
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        {/* ═══ 5. AI INSIGHTS (premium exclusive) ═══ */}
        {activeTab === "insights" && (
          <section>
            <div className="flex items-center gap-2 mb-5">
              <div className="w-1.5 h-6 bg-[#3ecf8e] rounded-full" />
              <h2 className="font-bold text-[19px] text-[#111827]">AI Insights</h2>
              <span className="text-[12px] text-[#9ca3af] ml-1">ការវិភាគ AI</span>
            </div>
...
// ─── Reusable Sub-Components (from pro/reports) ────────────────────

function PLCard({
  title, khmer, value, trend, isPositive, icon, accentColor, highlight = false,
}: {
  title: string; khmer: string; value: string; trend: string;
  isPositive: boolean; icon: React.ReactNode; accentColor: string; highlight?: boolean;
}) {
  return (
    <div className={`p-5 rounded-2xl border ${highlight ? "bg-[#3ecf8e] text-white border-transparent shadow-md" : "bg-white border-[#e8eaed] shadow-[0_1px_4px_rgba(0,0,0,0.06)]"}`}>
      <div className="flex justify-between items-start mb-4">
        <div>
          <h4 className={`text-sm font-semibold ${highlight ? "text-white/90" : "text-[#6b7280]"}`}>{title}</h4>
          <p className={`text-[11px] mt-0.5 ${highlight ? "text-white/70" : "text-[#9ca3af]"}`}>{khmer}</p>
        </div>
        <div className={`p-2 rounded-xl ${highlight ? "bg-white/20" : "bg-[#f0f2f5] text-[#6b7280]"}`}>
          {icon}
        </div>
      </div>
      <div className="flex items-end justify-between">
        <h2 className="text-[26px] font-bold tracking-tight leading-none">{value}</h2>
        <span className={`text-sm font-semibold mb-0.5 ${highlight ? "text-white" : isPositive ? "text-[#3ecf8e]" : "text-[#ef4444]"}`}>
          {trend}
        </span>
      </div>
    </div>
