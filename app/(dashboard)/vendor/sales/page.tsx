...
export default function SalesDashboard() {
  ...
  const handleSave = async () => {
    ...
    if (fInventoryItemId && !editTarget) {
    ...
    try {
      const url = editTarget
        ? `/api/vendor/sales/${editTarget.id}`
        : "/api/vendor/sales";
      ...
      if (result.success) {
        ...
        if (editTarget) {
          setTxns((p) =>
            p.map((x) => (x.id === editTarget.id ? mappedTxn : x)),
          );
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const res = await offlineFetch(`/api/vendor/sales/${deleteTarget.id}`, {
        method: "DELETE",
      });
      const result = await res.json();
      if (result.success) {
  ...
  return (
    <div
      className={`min-h-screen flex font-sans selection:bg-[#29B28D] transition-colors duration-200 ${isDark ? "bg-dark-bg text-[#e6edf3]" : "bg-slate-100 text-slate-900"}`}
    >
      {/* ── Toasts ── */}
        ...
        <VendorTopbar
          ...
          rightActions={
            <>
              ...
              >
                {(["Day", "Week", "Month"] as Period[]).map((p) => (
                  <button
                    key={p}
                    onClick={() => setPeriod(p)}
                    className={`px-4 py-[7px] rounded-[8px] text-[13px] font-semibold border-0 cursor-pointer transition-all ${period === p ? (isDark ? "bg-dark-surface text-white shadow-md" : "bg-white text-[#111827] shadow-sm") : "text-slate-500 hover:text-[#111827]"}`}
                  >
                    {p}
                  </button>
                ))}
              ...
              <button
                onClick={openAdd}
                className="flex items-center gap-[7px] bg-[#0d1117] text-[#29B28D] border border-[#29B28D]/20 rounded-[10px] px-4 py-[9px] font-bold text-[13px] shadow-[0_2px_14px_rgba(0,0,0,0.15)] hover:bg-black transition-all"
              >
          ...
          <div className="max-w-[1400px] mx-auto flex flex-col gap-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <VendorSummaryCard
                ...
                khmerTitle={t("dashboard.metrics.revenue")}
                value={`$${totalRevenue.toFixed(2)}`}
              />
              <VendorSummaryCard
                title={t("dashboard.metrics.sales")}
                khmerTitle={t("dashboard.metrics.sales")}
                value={txns.length}
                variant={isDark ? "dark" : "light"}
              />
              <VendorSummaryCard
                title={t("dashboard.metrics.avgSale")}
                khmerTitle={t("dashboard.metrics.avgSale")}
                ...
                <table className="w-full text-left">
                  <thead className="text-[10.5px] uppercase font-bold text-slate-500 border-b border-white/5">
                    <tr>
                      ...
                      <th className="px-[22px] py-[11px]">
                        {t("dashboard.table.method")}
                      </th>
                      <th className="px-[22px] py-[11px] text-right">
                        {t("dashboard.table.amount")}
                      </th>
                      <th className="px-[22px] py-[11px] text-center">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filtered.map((txn) => (
                      <tr
                        key={txn.id}
                        className="group hover:bg-white/5 transition-colors text-[13px]"
                      >
                        ...
                        <td className="px-[22px] py-[14px] font-medium">
                          {txn.items || "—"}
                        </td>
                        <td className="px-[22px] py-[14px]">
                          <span
                            className={`px-2.5 py-[3px] rounded-full text-[11px] font-bold ${METHOD_BADGE[txn.method]}`}
                          >
                            {txn.method}
                          </span>
                        </td>
                        <td className="px-[22px] py-[14px] text-right font-bold text-[#29B28D]">
                        ...
                        <td className="px-[22px] py-[14px] text-center relative">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setActiveMenuId(
                                activeMenuId === txn.id ? null : txn.id,
                              );
                            }}
      ...
      <ConfirmModal
        ...
        title={t("dashboard.modals.deleteSaleTitle")}
            ...
            <div className="bg-[#0d1117] px-7 py-5 flex items-center justify-between text-[#e6edf3]">
              <div>
                <div className="text-[18px] font-bold">
                  {editTarget
                    ? t("dashboard.modals.editSaleTitle")
                ...
                <div>
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
                    {inventory.length > 0 ? (
                        ...
                        (() => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span
                                className={`text-[11px] ${isSelected ? "text-[#0d1117]/70" : "text-slate-500"}`}
                              >
                      ...
                      <Link
                        href="/vendor/inventory"
                        className="flex items-center gap-2 px-4 py-3 rounded-xl border border-dashed border-slate-300 dark:border-white/10 text-slate-500 text-xs no-underline hover:border-[#29B28D] transition-all"
                      >
                        <Package size={14} />{" "}
                  ...
                  {fInventoryItemId && (
                    <div className="mt-4 flex items-center justify-between p-3 rounded-xl border border-[#29B28D]/30 bg-[#29B28D]/5 dark:bg-[#29B28D]/10">
                      ...
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className="w-8 h-8 rounded-full bg-white dark:bg-[#0d1117] border shadow-sm flex items-center justify-center text-slate-500 hover:text-[#29B28D]"
                        >
                          -
                        </button>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-9 pr-4 py-4 rounded-xl text-2xl font-bold border outline-none transition-all ${isDark ? "bg-[#0d1117] border-white/10 text-white focus:border-[#29B28D]" : "bg-slate-50 border-slate-200 text-slate-900 focus:border-[#29B28D]"} ${fInventoryItemId ? "opacity-50 cursor-not-allowed" : ""}`}
                  />
                </div>
              </div>
              <div>
                ...
                <input
                  type="text"
                  placeholder="e.g. 2x Phone Case"
                  value={fItems}
                  onChange={(e) => {
                    setFItems(e.target.value);
                    setFInventoryItemId(null);
                  }}
                  disabled={!!fInventoryItemId}
                  className={`w-full px-4 py-3.5 rounded-xl text-[14px] border outline-none transition-all ${isDark ? "bg-[#0d1117] border-white/10 text-white focus:border-[#29B28D]" : "bg-slate-50 border-slate-200 text-slate-900 focus:border-[#29B28D]"} ${fInventoryItemId ? "opacity-50 cursor-not-allowed" : ""}`}
                />
              </div>
              <div>
                <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2 block">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(["Cash", "ABA/KHQR", "Other"] as Method[]).map((m) => (
                    <button
                      ...
                      className={`py-3 rounded-[10px] text-[13px] font-bold transition-all ${fMethod === m ? "bg-[#29B28D] text-[#0d1117] shadow-lg" : isDark ? "bg-white/5 text-slate-400 border border-white/5 hover:bg-white/10" : "bg-slate-50 text-slate-500 border border-transparent hover:bg-slate-100"}`}
                    >
                      {m === "Cash"
                        ? t("dashboard.methods.cash")
                        : m === "ABA/KHQR"
                          ? t("dashboard.methods.qr")
