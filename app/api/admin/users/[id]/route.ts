...
export async function PATCH(
  ...
  try {
    ...
    if (action === "deactivate") {
      await db.update(users).set({ status: "blocked" }).where(eq(users.id, params.id));
    ...
    // Log the action
    const adminRecord = await db.query.admins.findFirst({ where: eq(admins.userId, actor.userId) });
    if (adminRecord) {
