...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: responseData, cached } = await withAICache(`weather-${vendor.id}`, CACHE_TTL, async () => {
      ...
      try {
        ...
        const weatherRes = await fetch("https://api.openweathermap.org/data/2.5/weather?q=Phnom+Penh&appid=4236df6aad6e0a04d15ad7a96cdc25a9&units=metric");
