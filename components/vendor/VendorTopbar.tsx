        ...
        <div className="hidden sm:flex items-center gap-2 pr-2 border-r border-[#e8eaed] dark:border-white/10 mr-1">
          ...
          <div className="relative" ref={notifRef}>
            ...
            >
              ...
              {unreadCount > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white dark:border-[#0d1117]" />}
            </button>
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-[#161B22] border border-[#e8eaed] dark:border-white/10 rounded-[14px] shadow-lg z-50 overflow-hidden text-left">
                <div className="p-4 border-b border-[#e8eaed] dark:border-white/10 flex justify-between items-center bg-[#f7f8fa] dark:bg-[#0d1117]">
                  ...
                  <span className="text-[11px] font-bold text-[#3ecf8e] bg-[rgba(62,207,142,0.15)] px-2.5 py-1 rounded-full border border-[rgba(62,207,142,0.3)]">{isKhmer ? `៣ ថ្មី` : `${unreadCount} New`}</span>
                </div>
                  ...
                  {allNotifications.map((notif, i) => (
                    <div key={i} className={`p-4 border-b border-[#e8eaed] dark:border-white/10 hover:bg-[#f7f8fa] dark:hover:bg-white/5 transition-colors cursor-pointer ${notif.unread ? "bg-[rgba(59,130,246,0.03)] dark:bg-[rgba(59,130,246,0.05)]" : ""}`}>
                      <div className="flex justify-between items-start mb-1">
                        <p className={`text-[13px] font-bold ${notif.type === 'error' ? 'text-red-600 dark:text-red-400' : 'text-[#111827] dark:text-white'}`}>{notif.title}</p>
                        {notif.unread && <span className={`w-2 h-2 ${notif.type === 'error' ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]' : 'bg-[#3b82f6] shadow-[0_0_8px_rgba(59,130,246,0.6)]'} rounded-full mt-1 shrink-0`}></span>}
                      </div>
